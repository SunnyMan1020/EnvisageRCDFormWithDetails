...
export default function FormPopup(props) {
  ...
  useEffect(() => {
    async function combinedFetch() {
      return Promise.all([
        ...
        fetch("https://envisagepj005.azurewebsites.net/getRcdOperator")
          .then((value) => value.json())
          .then((data) => setOperatorList(data)),
  ...
  useEffect(() => {
    if (
      ...
      operatorList.forEach((prj) => {
        operatorNameAnswerChoices.push(prj.Name);
      });
  ...
  }, [projectNameList, plantList, operatorList]);
              ...
              <Fragment>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {recordsOptions.Operators.length > 0 &&
                        recordsOptions.Operators.map((Id) => {
                          return (
                            <MenuItem value={Id} key={Id}>
                              {Id}
                            </MenuItem>
